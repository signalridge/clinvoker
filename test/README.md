...
│   ├── test_openai.sh
│   ├── test_anthropic.sh
│   ├── test_mcp.sh        # MCP HTTP protocol behavior
│   └── test_mcp_auth.sh   # MCP API key + SSE auth behavior
...
| `MCP_SERVER_PORT` | `18083` | MCP HTTP test server port |
| `MCP_SERVER_PATH` | `/mcp` | MCP HTTP endpoint path |
...
- `start_mcp_http_server` / `stop_mcp_http_server` - Manage MCP HTTP test server
...
| `test_mcp.sh` | MCP JSON-RPC over HTTP (initialize/tools/ping/notification/SSE) |
| `test_mcp_auth.sh` | MCP endpoint API key auth + SSE auth behavior |
