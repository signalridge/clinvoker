...
> A session must have a backend session ID to be resumable. Sessions created in `--ephemeral` mode cannot be resumed.
...
This resumes the most recent resumable session (not scoped to directory unless you use `--here` with `resume`).
...
```yaml
session:
  # Automatically resume the most recent resumable session when running `clinvk [prompt]`
  auto_resume: true
