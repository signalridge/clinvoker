...
| `--all-backends` | Compare all enabled backends | `false` |
| `--sequential` | Run one at a time | `false` |
| `--json` | JSON output | `false` |

## Configuration

Enable or disable backends for comparison in `~/.clinvk/config.yaml`:
...
backends:
  ...
  codex:
    enabled: true
  gemini:
    enabled: false  # Won't appear in --all-backends
```
